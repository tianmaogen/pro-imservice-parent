...
public class SignUtil {
	...
	public static String getSign(Long skdAppid, String account, String privStr, String pubStr) {
		try{
//			//Use pemfile keys to test
//			String privStr1 = "-----BEGIN PRIVATE KEY-----\n"+
...
//			System.out.println("---\ngenerate sig:\n" + result.urlSig + "\n---\n");
//			System.out.println("---\ngenerate sig:\n" + result.urlSig.length() + "\n---\n");

			// check signature
			CheckTLSSignatureResult checkResult = CheckTLSSignatureEx(result.urlSig, skdAppid, account, pubStr);
			if(checkResult.verifyResult == false) {
//				System.out.println("CheckTLSSignature failed: " + result.errMessage);
				return "ERROR";
			}
//			System.out.println("\n---\ncheck sig ok -- expire time " + new Date(checkResult.expireTime) + " -- init time " + new Date(checkResult.initTime) + "\n---\n");
	...
	public static void main(String[] args) {
		try{
			...
			"MIGEAgEAMBAGByqGSM49AgEGBSuBBAAKBG0wawIBAQQgrST5Cp0xqvrZH3eeCUJa\n" +
			"fpCZkZtMiE8FTZRf+QwGDJuhRANCAARsE2+zvnRysR6eC/HNPZ/xA8a5m3OKLt1K\n" +
			"lyWsa7IWZR7VFR1DGDm49VppLUrZ0HothhS++d3ZDQiaVAEkKvCr\n" +
			"-----END PRIVATE KEY-----";

			//change public pem string to public string
			String pubStr = "-----BEGIN PUBLIC KEY-----\n"+
			"MFYwEAYHKoZIzj0CAQYFK4EEAAoDQgAEbBNvs750crEengvxzT2f8QPGuZtzii7d\n"+
			"SpclrGuyFmUe1RUdQxg5uPVaaS1K2dB6LYYUvvnd2Q0ImlQBJCrwqw==\n"+
			"-----END PUBLIC KEY-----";

			System.out.println(privStr);
            // generate signature 有效期为30年
            GenTLSSignatureResult result = GenTLSSignatureEx(1400014964, "ibaby", privStr, 3600*24*180);
            if (0 == result.urlSig.length()) {
                System.out.println("GenTLSSignatureEx failed: " + result.errMessage);
                return;
            }
...
//			System.out.println("---\ngenerate sig:\n" + result.urlSig.length() + "\n---\n");
            ...
            // check signature
            CheckTLSSignatureResult checkResult = CheckTLSSignatureEx(result.urlSig, 1400014964, "ibaby", pubStr);
            if(checkResult.verifyResult == false) {
                System.out.println("CheckTLSSignature failed: " + result.errMessage);
                return;
            }
